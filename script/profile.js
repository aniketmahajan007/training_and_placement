...
$(document).ready(function(){
    var this_form_has_login_device=new FormData($('#logout')[0]);
    $.ajax({
                 url: "http://pccoetnp.epizy.com/handler/sync_manager/2",
                ...
                data: this_form_has_login_device,
                 ...
                 success: function(data) {
                     ...
                     if(data.placed_status==1){
                         alert("Congratulation! You have been placed in "+data.placed_in+"                              Note: A placed student cannot view or edit their profile, click ok button to log out")
                         document.cookie = 'login_cookies=; expires=Thu, 01 Jan 1970 00:00:01 GMT;';
                         window.location.href = "http://pccoetnp.epizy.com/";
                         return 0;
                     }
    ...
    $("#u_submit").click(function(e){ e.preventDefault();
            var update_profile_form_data=new FormData($('#update_profile')[0]);
        $("#loading").show();
        $.ajax({
                 url: "http://pccoetnp.epizy.com/handler/sync_manager/3",
                 headers: { 'Authority': '$2y$08$gFWXSf4TLya4YH1co4M6.uQRT1JUeTLednPmIwwWeIRwrpKwP2K2m' ,
                  'Tnpbot' : cookie_token},
                data: update_profile_form_data,
                 crossDomain: true,
                 processData: false,
                 contentType: false,
                 timeout:30000,
                 ...
                 success: function(data)
                  {
                     ...
                     if(data.status==="fields"){
                         alert("Invalid inputs, please check for any mistakes.")
                     }
                     
                     if(data.status==="yearofcomplete"){
                         alert("Invalid year of completion.")
                     }
                     
                     if(data.status==="sscyear"){
                         alert("Invalid SSC year Input.")
                     }
                                          
                     if(data.status==="hscyear"){
                         alert("Invalid HSC year Input.")
                     }
                                          
                     if(data.status==="gradyear"){
                         alert("Invalid Graduation year Input.")
                     }
                                          
                     if(data.status==="mob_number"){
                         alert("Invalid mobile number.")
                     }
                                          
                     if(data.status==="f_email"){
                         alert("Invalid father's email.")
                     }
                                          
                     if(data.status==="father_number"){
                         alert("Invalid Father's Number.")
                     }
                 }                
         });
...
function ValidateImage(_validFileExtensions,type_checker) {
    var arrInputs = type_checker;
    for (var i = 0; i < arrInputs.length; i++) {
        ...
        if (oInput.type === "file") {
            ...
            if (sFileName.length > 0) {
                ...
                if (!blnValid) {
                    alert("Image not supported, please select image with : " + _validFileExtensions.join(", "));
                    return false;
                }
            }
        }
...
$(document).ready(function (){
    document.getElementById("p_sub").disabled=true;
    document.getElementById("r_sub").disabled=true;
})
$(".profile_user_photo_left").change(function (){
    document.getElementById("p_sub").disabled=false;
});
$("#profile_user_resume_right").change(function (){
    document.getElementById("r_sub").disabled=false;
});
$("#r_sub").click(function (e){
    e.preventDefault();
    var pval=$('#profile_user_resume_right').val();
    if(pval!==""){
        var this_form_data=new FormData($('#s_resume_update')[0]);
        $("#loading").show();
        $.ajax({
            url: "http://pccoetnp.epizy.com/handler/sync_manager/5",
            headers: { 'Authority': '$2y$08$gFWXSf4TLya4YH1co4M6.uQRT1JUeTLednPmIwwWeIRwrpKwP2K2m' ,
                'Tnpbot' : cookie_token},
            data: this_form_data,
            crossDomain: true,
            ...
            success: function(data)
            {
                ...
                if(data.status==="fields"){
                    alert("Please Select pdf format file before submitting");
                }else if(data.status==="size"){
                    alert("Image size should be smaller than 2 M.B.")
                }else if(data.status==="format"){
                    alert("Only PDF format file are supported.");
        ...
        alert("Please select image first before submitting");
    }
});
$("#p_sub").click(function (e){
    e.preventDefault();
    var pval=$('.profile_user_photo_left').val();
    if(pval!==""){
        if(!ValidateImage([".jpg", ".jpeg", ".png"],$(".profile_user_photo_left"))){
            return false;
        }
        var this_form_data=new FormData($('#s_photo_update')[0]);
        $("#loading").show();
        $.ajax({
            url: "http://pccoetnp.epizy.com/handler/sync_manager/4",
            headers: { 'Authority': '$2y$08$gFWXSf4TLya4YH1co4M6.uQRT1JUeTLednPmIwwWeIRwrpKwP2K2m' ,
                'Tnpbot' : cookie_token},
            data: this_form_data,
            crossDomain: true,
            ...
            success: function(data)
            {
                ...
                if(data.status==="fields"){
                    alert("Please Select image before submitting");
                }else if(data.status==="size"){
                    alert("Image size should be smaller than 1 M.B.")
                }else if(data.status==="format"){
                    alert("Image not supported, please select image with : JPG, JPEG, PNG");
                }else if(data.status==='success'){
                    alert("Successfully Updated.");
                    $("#user_profile_picture").attr('src',data.profile_avatar);
                }
